  ...
  return {
    ...
    GetPercentageFromMiddleware: function() {
      ...
      var products = JSON.parse(
        decodeURIComponent(jQuery(".co2ok_container").attr("data-cart"))
      );
      ...
      promise.then(function(percentage) {
        ...
        jQuery.post(ajax_object.ajax_url, data, function(response) {
          jQuery(".compensation_amount").html(
    ...
    RegisterBindings: function() {
      ...
      jQuery("#co2ok_cart, #checkbox_label, .co2ok_checkbox_container").click(
        function(event) {
    ...
    RegisterInfoBox: function() {
      ...
      var reference = document.querySelector(".co2ok_info");
      var popper = document.querySelector(".co2ok_infobox_container");

      jQuery(".co2ok_infobox_container").hide();

      jQuery(".co2ok_info").click(function() {
        jQuery(".co2ok_infobox_container").show();
        ...
        var element = ".co2ok-popper";

        jQuery("body").on("click", function(e) {
          //Use each to hide Popovers with the same class
          jQuery(element).each(function(index, elm) {
            hidePopover(elm, e);
          });
        });
